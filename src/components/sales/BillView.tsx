      ...
      >
        ...
        <div className="relative z-10">
          <div className="text-center border-b-2 border-slate-100 pb-6 mb-6">
            ...
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider">Sale Invoice</p>
            <div className={`mt-4 inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
              ...
              <CheckCircle2 className="h-3 w-3 mr-1" /> {(bill.status || 'unpaid').toUpperCase()}
            </div>
          </div>

          <div className="grid grid-cols-2 gap-y-4 text-sm mb-8">
            <div>
              <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Invoice Number</p>
              <p className="font-bold text-slate-900">{bill.billNumber || 'N/A'}</p>
            </div>
            <div className="text-right">
              <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Date</p>
              <p className="font-bold text-slate-900">{bill.date || 'N/A'}</p>
            </div>
            <div>
              ...
              <span className={cn(
                ...
                bill.paymentType === 'credit' ? "text-rose-700" : "text-emerald-700"
              )}>
                {bill.paymentType === 'credit' ? 'Credit (Udhar)' : 'Cash'}
              </span>
            </div>
            <div className="text-right">
              <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Due Date</p>
              <p className="font-bold text-slate-900">{bill.dueDate || 'N/A'}</p>
            </div>
            <div className="col-span-2">
              <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Billed To (Buyer)</p>
              <p className="font-bold text-slate-900 text-lg leading-tight">{bill.buyerName || 'Unknown Buyer'}</p>
            </div>
          </div>
          ...
          <div className="bg-slate-50 rounded-lg p-4 mb-6 border border-slate-100">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Item</p>
                <p className="font-bold text-slate-900 leading-tight">{bill.itemName || 'Rice Item'}</p>
              </div>
              <div className="text-right">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-slate-600 border-t border-slate-200 pt-3">
              ...
              <span className="text-right font-medium">{(bill.weight || 0).toFixed(2)} kg</span>
              <span>Sale Rate ({bill.rateType === 'per_kg' ? 'per kg' : 'per katta'}):</span>
              <span className="text-right font-medium">{formatCurrency(bill.rate)}</span>
              {(Number(bill.bhardana) > 0) && (
                <>
                  <span>Bhardana:</span>
          ...
          <div className="bg-slate-900 text-white rounded-lg p-5">
            <div className="flex justify-between items-center mb-2">
              ...
              <p className="text-xl font-bold">{formatCurrency(bill.totalAmount || 0)}</p>
            </div>
            <div className="flex justify-between items-center border-t border-slate-700 pt-2 text-xs">
              ...
              <span className="font-bold text-emerald-400">{formatCurrency(bill.paidAmount || 0)}</span>
            </div>
